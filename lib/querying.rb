...
def select_series_title_with_most_human_characters
  "SELECT series.title
   ...
   WHERE species = 'human' GROUP BY(series.title) ORDER BY(species) DESC LIMIT 1;
   "
end
...
def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, COUNT(books.title)
   ...
   JOIN books
   ON character_books.book_id = books.id GROUP BY(characters.name) ORDER BY(COUNT(books.title)) DESC;"
end
